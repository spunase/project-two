...
brands = ["Alabama", "Alaska", "Arizona", "Arkansas", "California", "Colorado", "Connecticut", "Delaware", 
...
var csvdata = d3.csv("final_dataset_101520.csv").then(function(data, err) {
  if (err) throw err;
  data.map(function(d) { 
    if (d.year == 2019)  {
    states.push(d.state);}
    values.push(+d.house_median_value);
    // years.push(+d.year);
   });
   console.log(values);
   console.log(states);
  });
  var mapArrayWithIndex = states.map(function(item, index) {
